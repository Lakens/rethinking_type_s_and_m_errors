...
#| fig-cap: "Comparison of the observed effect sizes (green, with the grey area illustrating missing non-significant values), and bias-adjusted effect sizes based on an incorrect use of the Type M exaggeration ratio (yellow), and p-uniform (brown)."
#| warning: false
#| message: false
#| echo: false
#| fig-align: center
...
A second use-case of a Type S and M error is in highly exploratory research where researchers opportunistically search for statistically significant effects without correcting for multiple comparisons. The effects that researchers explore in such a scenario will include very small effect sizes. As the study is not designed to detect such small effects, statistical power for many exploratory tests will be very low. For example, if researchers design a study to have 80% power for a Cohen's *d* of 0.5 in an independent *t*-test. Only effects larger than *d* = `r round(imperfect_science_example_critical_es$dc, 3)` will be significant. Exploratory tests in which researchers selectively focus on statistically significant effects will on average lead to exaggerated effect size estimates. If researchers explore small effects where the true Cohen's *d* is 0.25, or even 0.15, significant effects will on average be exaggerated by a factor of `r round(typeM_imperfect_science_1, 2)` or `r round(typeM_imperfect_science_2, 2)`, respectively. Researchers could report the Type M error to warn readers that in addition to a high probability that significant effects are false positives, effect sizes are likely to be overestimated, and remind readers that results from exploratory analyses should be treated as hypotheses that need to be severely tested in follow-up studies [@ditroilo_exploratory_2025].
